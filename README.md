...
The **eastern-redness Maghrib** (Maghrib at the preset's depression angle, ~13–18 min after sunset under Leva Qom and ~15–21 min under Tehran depending on latitude/season) is the explicit Ja'fari distinction from Sunni Maghrib at sunset.

**Sunrise/sunset use the apparent horizon (−0.833°)** — atmospheric refraction (~34′) + solar semi-diameter (~16′) — matching adhan.js. The other four thresholds (Fajr, Maghrib, and Isha at the preset's depression angles per the table above; Asr at shadow-factor 1) are taken geometrically, again matching adhan.js: refraction is negligible at those depths. See the commit message on `bf85d01` for the per-threshold audit.

**Shar'ī midnight** (the moment Isha's *waqt* ends) is computed as the midpoint of (Maghrib, next Fajr) at each preset's respective depression angles, the canonical Ja'fari definition per Sistani / Khamenei tawḍīḥ. This determines when `classifyPrayer` switches from the afternoon branch (Asr → Maghrib → Isha) to the morning branch (Fajr → Sunrise). The same anchoring is used by the high-latitude methods 4 (niṣf al-layl) and 5 (sub'iyya), with a documented sunrise fallback when Fajr is physically unreachable at the user's latitude/date. Method 6 (angle-based with seasonal reduction) computes reduced-angle Fajr/Isha directly and only inherits the next-Fajr anchor transitively, via its fall-through to method 4 in polar-day / ordering edge cases. See [High-latitude methods](#high-latitude-methods).
...
| 4 | **Niṣf al-layl — middle of night** | Define night as (Maghrib → next Fajr); Isha and Fajr fold to the midpoint. When the next Fajr isn't physically reachable (sun never reaches the preset's Fajr depression — −16° Leva Qom, −17.7° Tehran — at this latitude/date), falls back to next sunrise — the panel marks the substitution. | The Ja'farī rendering of the classic high-latitude rule. Matches the canonical shar'ī midnight (½(Maghrib + next Fajr)) used elsewhere in the app. In deep polar summer Isha and Fajr collapse to the same instant — the panel displays them as such. |
| 5 | **Sub'iyya — one-seventh** | Night = (Maghrib → next Fajr). Isha at first 1/7 past Maghrib; Fajr at last 1/7 before end-of-night. Sunrise fallback when Fajr (at the preset's depression angle) is unresolvable, surfaced in the panel. | Symmetric, predictable, used by several Sunni and Shia conventions. Diverges from adhan.js's `HighLatitudeRule.SeventhOfTheNight` (sunrise-anchored) to match the Ja'farī shar'ī-midnight definition. |
| 6 | **Angle-based with seasonal reduction** | Use the preset's configured Fajr / Isha angles (Leva Qom −16°/−14°, Tehran −17.7°/−14°) when the sun reaches them; otherwise reduce toward whatever the sun actually achieves at solar midnight, so Fajr and Isha are always defined. | Converges to the standard rule when conditions allow; converges to niṣf al-layl (next-Fajr anchored) in deep polar summer. |
...
- **Methods 4 (niṣf al-layl) and 5 (sub'iyya) now anchor "end of night" on next Fajr** (the canonical Ja'farī shar'ī midnight, ½(Maghrib + next Fajr)) instead of next sunrise. Method 6 (angle-based with seasonal reduction) keeps its own reduced-angle logic and inherits the new anchor only via its existing fall-through to method 4 in polar-day / ordering edge cases. When Fajr is physically unreachable at the user's latitude/date the sunrise fallback still applies and the panel discloses it ("end of night derived from next sunrise"). Existing share-links (`?m=4|5|6`) continue to work; only the anchor changed.
