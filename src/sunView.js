...
export function createSunView({ earthMaterial, sunGroup, sunLine, sunTrace, subsolarTrace, getDateOffsetMs, markDirty }) {
  ...
  function resampleTraces() {
    // Anchor the 24h window to the scrubbed date (date offset only) so the ring
    // migrates between the tropics with declination; the marker (effectiveNow)
    // slides along it as the intra-day time is scrubbed.
    const center = Date.now() + getDateOffsetMs();
