...
// integer ones byte-exact; the 50/60 fractions to within 1e-12 radians).
...
eq("FAJR_LIMIT_DEG", FAJR_LIMIT_DEG, 74);
eq("DAY_LIMIT_DEG", DAY_LIMIT_DEG, 90 + 50 / 60);
eq("SAFE_MARGIN_DEG", SAFE_MARGIN_DEG, 0.05);
...
has("const float FAJR_LIMIT = 74.0 * PI / 180.0;");
has("const float SAFE_MARGIN = 0.05 * PI / 180.0;");
has("max(cosEff * cosDec, 0.0001)");

// Fractional thresholds: confirm the GLSL evaluates to the original radians.
eq("APPARENT_HORIZON rad", (APPARENT_HORIZON_DEG * Math.PI) / 180, (-50 / 60 * Math.PI) / 180);
eq("DAY_LIMIT rad", (DAY_LIMIT_DEG * Math.PI) / 180, ((90 + 50 / 60) * Math.PI) / 180);
...
if (fail.length) {
  ...
  console.log("shaderConstants: 16/16 passed");
}
